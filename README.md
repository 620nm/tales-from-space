...
| Directory        | Contents |
| ---------------- | -------- |
| `content/`       | The mod itself: `items/`, `jobs/`, `bodies/`, `gamemodes/`, `fixtures/`, `substances/`, `reactions/`, `air/` (`.luau` files return prototype data and register behavior through explicit `Definition:handle` calls), plus `tuning.luau` feel knobs. |
