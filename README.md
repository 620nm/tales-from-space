...
| Power | `t.cable_at(x, y)` (the colour mask of every PIECE under a tile, a run that merely crosses it included), `t.cable_segments(x, y) -> {{colour, d1, d2}, …}` (those pieces themselves, in draw order — docs/POWER.md), `t.apc_at(x, y) -> {charge, equipment, lighting, environment, alarmed, charging}`, `t.segment_at(x, y) -> {l1, l2, l3, live, feed, lighting_feed}` (the circuit id for each colour with a KNOT under the tile, so a run that merely crosses answers nothing, and which colour BIT a consumer here would bind to per channel), `t.ports_at(x, y) -> {kind, energized, l1 = {mode, in_w, out_w}, l2 =, l3 =}` (a bridge's ports, one per colour with a knot under it), `t.lit(x, y)` |
