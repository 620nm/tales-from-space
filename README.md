...
| Atmos | `t.pipe(x, y)`, `t.pipe_pressure(x, y [, layer])`, `t.room_pressure(x, y)`, `t.canister_pressure(x, y)`, `t.deck(x, y) -> {capacity, temperature, energy}` (the solid mass a room stands on — J/K, K, J; plating reports an infinite capacity and no energy), `t.wind(x, y) -> east, north` (what the air over a tile is doing, m/s on the map's axes; a wall answers 0, 0) |
