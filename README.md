...
| Power | `t.apc_at(x, y)` (charge and device channels), `t.network_segment(x, y, network, channel) -> {segment, equipment, lighting}`, `t.network_feed(x, y, network, service)` (the authored channel feeding equipment or lighting), `t.outlet_at(x, y)` (network, channel-keyed LEDs, and feed identity), `t.ports_at(x, y, network) -> {kind, energized, channels}` (port readouts keyed by authored channel id), `t.lit(x, y)` |
