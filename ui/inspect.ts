...
function inspectionCard(look: Inspection, history = false): UiNode {
  ...
  return {
    ...Pane(id, [
      row(`${id}/head`, some(
        icon(`${id}/icon`, look.sprite),
        text(`${id}/title`, look.title, ["titlebar-title", "grow"]),
        ...
        press(`${id}/close`, S.CLOSE_MARK, () => { dismissed.add(look.sequence); pins.delete(look.sequence); return undefined; }, { variant: "ghost" }),
      ), { style: { gap: 6, alignItems: "center" } }),
      column(`${id}/lines`, examineLines(look.lines ?? [], id), { style: { gap: 3 } }),
    ], { cls: ["inspect-toast"], style: { maxHeight: 220 } }),
    ...(!pinned && !history ? { expires: 8450, fade: 450, pauseExpiry: true } : {}),
  };
}
...
export function inspectionPanels(view: GameplayView): UiNode[] {
  ...
  if (identity !== state.identity?.you || !state.inspections?.length) {
    pins.clear(); dismissed.clear(); historyOpen = false; identity = state.identity?.you;
  ...
  if (records.length) out.push(column("inspection-stack", [
    press("inspection-history", S.tfs("ui.look.history"), () => { historyOpen = !historyOpen; return undefined; }, { variant: historyOpen ? "selected" : "ghost" }),
    ...recent.map((look) => inspectionCard(look)),
  ], { cls: ["hudgroup"], style: { position: "absolute", left: 14, top: 18, width: 320, maxHeight: "66%", overflowY: "auto", gap: 6 } }));
  ...
  if (state.hover) {
    ...
    out.push({ ...Pane("hover", [
      row("hover/title", [
        ...
        text("hover/name", hover.name, ["titlebar-title"]),
